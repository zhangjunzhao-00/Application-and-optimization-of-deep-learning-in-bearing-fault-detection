...
model_name = "predict-1-LSTM"
# 实例化一个Sequential
model = Sequential()
#第一层LSTM
model.add(LSTM(80, activation='tanh', recurrent_activation='hard_sigmoid', kernel_initializer='glorot_uniform', recurrent_initializer='orthogonal',
                    bias_initializer='zeros', return_sequences=True))
model.add(Dropout(0.2))
# 第二层LSTM
model.add(LSTM(100, recurrent_activation='hard_sigmoid', return_sequences=True))
model.add(Dropout(0.2))
#第三层LSTM
model.add(LSTM(120, recurrent_activation='hard_sigmoid', return_sequences=False))
model.add(Dropout(0.2))
...
logdir = os.path.join('.\logs\predict-LSTM-1_logs')
...
if checkpoint_flag :
    print('开始断点续训')
    checkpoint_save_path = "./checkpoint/predict-1-LSTM.ckpt"
