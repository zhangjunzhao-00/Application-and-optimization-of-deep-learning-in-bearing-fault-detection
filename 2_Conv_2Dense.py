...
#第二层卷积
model.add(Conv1D(filters=64, kernel_size=5, strides=2,padding='same',kernel_regularizer=l2(1e-4)))
model.add(BatchNormalization())
model.add(Activation('relu'))
...
if checkpoint_flag :
    checkpoint_save_path = "./checkpoint/2_Conv_2Dense.ckpt"
...
target_path = r'data\2HP' #使用马力=1的数据集作为目标领域
