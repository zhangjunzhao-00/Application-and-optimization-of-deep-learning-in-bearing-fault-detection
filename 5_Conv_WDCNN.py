...
AdaBN=True  #是否使用AdaBN操作，若使用：False；若不使用：True
BN_momentum=0.98  #BN层移动平均权重
Dropout_flag=0.2  #Dropout层参数
...
model_name = "5_Conv_WDCNN"

# 实例化一个Sequential
model = Sequential()

model.add(Dropout(Dropout_flag))
#第一层卷积
model.add(Conv1D(filters=16, kernel_size=128, strides=32, padding='same',kernel_regularizer=l2(1e-4), input_shape=input_shape))
model.add(BatchNormalization(trainable=AdaBN,momentum=BN_momentum))
model.add(Activation('relu'))
model.add(MaxPooling1D(pool_size=2, strides=2, padding='valid'))
model.add(Dropout(Dropout_flag))

#第二层卷积
model.add(Conv1D(filters=32, kernel_size=3, strides=1,padding='same',kernel_regularizer=l2(1e-4)))
model.add(BatchNormalization(trainable=AdaBN,momentum=BN_momentum))
model.add(Activation('relu'))
model.add(MaxPooling1D(pool_size=2, strides=2, padding='valid'))
model.add(Dropout(Dropout_flag))
...
#第三层卷积
model.add(Conv1D(filters=64, kernel_size=3, strides=1,padding='same',kernel_regularizer=l2(1e-4)))
model.add(BatchNormalization(trainable=AdaBN,momentum=BN_momentum))
model.add(Activation('relu'))
model.add(MaxPooling1D(pool_size=2, strides=2, padding='valid'))
model.add(Dropout(Dropout_flag))
...
#第五层卷积
model.add(Conv1D(filters=128, kernel_size=3, strides=1,padding='same',kernel_regularizer=l2(1e-4)))
model.add(BatchNormalization(trainable=AdaBN,momentum=BN_momentum))
model.add(Activation('relu'))
model.add(MaxPooling1D(pool_size=2, strides=2, padding='valid'))
model.add(Dropout(Dropout_flag))
...
#第六层卷积
model.add(Conv1D(filters=128, kernel_size=3, strides=1,padding='valid',kernel_regularizer=l2(1e-4)))
model.add(BatchNormalization(trainable=AdaBN,momentum=BN_momentum))
model.add(Activation('relu'))
model.add(MaxPooling1D(pool_size=2, strides=2, padding='valid'))
model.add(Dropout(Dropout_flag))
...
if checkpoint_flag :
    print('开始断点续训')
    checkpoint_save_path = "./checkpoint/5_Conv_WDCNN.ckpt"
